...
import com.algoviz.plus.features.auth.presentation.util.GoogleSignInHelper
import com.algoviz.plus.features.auth.presentation.util.GoogleSignInResult
...
fun LoginScreen(
    ...
    val googleSignInLauncher = rememberLauncherForActivityResult(
        ...
        when (val signInResult = GoogleSignInHelper.handleSignInResult(result, context.packageName)) {
            is GoogleSignInResult.Success -> viewModel.signInWithGoogle(signInResult.idToken)
            ...
            is GoogleSignInResult.Error -> {
                scope.launch {
                    snackbarHostState.showSnackbar(signInResult.message)
                }
            }
        }
    }

    val webClientId = remember(context.packageName) { resolveWebClientId(context) }
    val googleSignInClient = remember(webClientId) {
        if (webClientId.isBlank()) {
            null
        } else {
            GoogleSignInHelper.getGoogleSignInClient(context, webClientId)
        }
    }
    ...
    Box(
        ...
        Scaffold(
            ...
            Column(
                ...
                Text(
                    text = "Sign in to continue your algorithmic journey.",
                ...
                GoogleSignInButton(
                    onClick = {
                        ...
                        GoogleSignInHelper.launchSignIn(client) { signInIntent ->
...
private fun resolveWebClientId(context: android.content.Context): String {
    ...
    // Prefer the hosting app module resource (generated by google-services or checked in).
    val appPackageName = context.packageName
    val appResourceId = context.resources.getIdentifier(
        "default_web_client_id",
        "string",
        appPackageName
    )
    if (appResourceId != 0) {
        return context.getString(appResourceId)
    }
